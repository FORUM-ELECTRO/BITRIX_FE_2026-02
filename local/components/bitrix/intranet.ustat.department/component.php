...
switch ($arParams['PERIOD'])
{
	...
	// Принудительно добавляем года 2024, 2025
	case '2024':
		$fromDate = Type\DateTime::createFromTimestamp(mktime(0, 0, 0, 1, 1, 2024));
		$toDate = Type\DateTime::createFromTimestamp(mktime(23, 59, 59, 12, 31, 2024));
		$interval = 'month';
		$dateFormat = 'Y-m';
		$axisDateFormat = "M\nY";
		$axisCursorDateFormat = "F\nY";
	...
	case '2025':
		$fromDate = Type\DateTime::createFromTimestamp(mktime(0, 0, 0, 1, 1, 2025));
		$toDate = Type\DateTime::createFromTimestamp(mktime(23, 59, 59, 12, 31, 2025));
		$interval = 'month';
		$dateFormat = 'Y-m';
		$axisDateFormat = "M\nY";
		$axisCursorDateFormat = "F\nY";
...
// Исключаем бота 1668 везде
$excludeUserId = 1668;

// 1. Убираем из рейтинга (top)
if (isset($usersRating['top']) && is_array($usersRating['top']))
{
    $newTop = [];
    foreach ($usersRating['top'] as $item)
    {
        if ($item['USER_ID'] != $excludeUserId)
        {
            $newTop[] = $item;
        }
    }
    $usersRating['top'] = $newTop;
}

// 2. Убираем из данных активности (data)
